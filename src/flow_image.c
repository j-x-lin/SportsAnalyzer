...
image make_integral_image(image im)
{
    ...
    for (int k = 0; k < im.c; k++)
    {
        ...
        {
            sum += get_pixel(im, 0, j, k);
            set_pixel(integ, 0, j, k, sum);
        }
        ...
        for (int j = 1; j < im.h; j++)
        {
            ...
            {
                set_pixel(integ, i, j, k, get_pixel(integ, i, j-1, k) + get_pixel(integ, i-1, j, k) - get_pixel(integ, i-1, j-1, k) + get_pixel(im, i, j, k));
            }
        }
    }
    ...
    for (k = 0; k < im.c; k++)
    {
        for (j = 0; j < im.h; j++)
        {
            for (i = 0; i < im.w; i++)
            {
                ...
                rb = get_pixel(integ, right, bottom, k);
                ...
                if (left < 0) {
                    ...
                    lb = get_pixel(integ, left, bottom, k);
                    lt = get_pixel(integ, left, top, k);
                }
                ...
                if (top < 0) {
                    ...
                    lt = get_pixel(integ, left, top, k);
                    rt = get_pixel(integ, right, top, k);
                }

                float sum = rb+lt-rt-lb;
                int area = (right-left) * (bottom-top);

                // printf("%d %d %d %d %f %f %f %f %f %d %f %d\n", left, right, bottom, top, lt, rt, lb, rb, sum, area, sum/area, s);

                set_pixel(S, i, j, k, sum/area);
                // return S;
            }
        }
    }
...
image time_structure_matrix(image im, image prev, int s)
{
    ...
    for (j = 0; j < im.h; j++)
    {
        for (i = 0; i < im.w; i++)
        {
            ...
            set_pixel(S, i, j, 3, Ix_ij * get_pixel(diff, i, j, 0));
            set_pixel(S, i, j, 4, Iy_ij * get_pixel(diff, i, j, 0));
        }
    }
...
image velocity_image(image S, int stride)
{
    ...
    for(j = (stride-1)/2; j < S.h; j += stride){
        for(i = (stride-1)/2; i < S.w; i += stride){
            ...
            matrix dt = make_matrix(2, 1);
            dt.data[0][0] = -Ixt;
            dt.data[1][0] = -Iyt;            

            matrix res = matrix_mult_matrix(matrix_invert(M), dt);
            vx = res.data[0][0];
            vy = res.data[1][0];
